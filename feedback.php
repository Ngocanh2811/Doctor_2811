...
// Count unanswered for notification
$sqlNew = "SELECT COUNT(*) AS cnt FROM patient_questions WHERE DoctorID = ? AND IsAnswered = 0";
$nstmt = $conn->prepare($sqlNew);
$nstmt->bind_param('i', $doctorId);
$nstmt->execute();
$newCount = $nstmt->get_result()->fetch_assoc()['cnt'];
?>
<!DOCTYPE html>
<html lang="vi">
<head>
  ...
  <style>
    ...
    .sidebar .nav-link.active, .sidebar .nav-link:hover { background:#95b8ff; color:#fff; }
...
<div class="d-flex">
  ...
  <nav class="sidebar d-flex flex-column">
    ...
    <ul class="nav nav-pills flex-column mt-3">
      ...
      <li class="nav-item"><a href="patient_questions.php" class="nav-link active">
        Phản hồi thắc mắc <?php if($newCount){echo '<span class="badge badge-new ms-1">'.$newCount.'</span>';}?>
      </a></li>
    ...
    <div class="content">
      <div class="card-container mb-4">
        <table id="qTable" class="table table-striped">
          <thead>
            <tr>
              <th>Bệnh nhân</th><th>Câu hỏi</th><th>Ngày giờ</th><th>Trạng thái</th><th>Hành động</th>
            </tr>
          </thead>
          <tbody>
          <?php while($row = $questions->fetch_assoc()): ?>
            <tr data-id="<?= $row['QuestionID'] ?>"
              ...
              <td>
                <?= $row['IsAnswered'] ? '<span class="badge bg-success">Đã trả lời</span>'
