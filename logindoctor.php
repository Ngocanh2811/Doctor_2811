...
// Nếu đã login rồi, chuyển tiếp ngay và dừng script
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if (empty($username) || empty($password) || empty($role)) {
    $error_message = 'Please enter all required fields'; $valid = false;
  } elseif ($role === 'admin'  && empty($admin_code))     {
    $error_message = 'Admin authentication code is required';    $valid = false;
  } elseif ($role === 'doctor' && empty($doctor_license)) {
    $error_message = 'Medical license ID is required';        $valid = false;
  } elseif ($role === 'nurse'  && empty($nurse_id))       {
    $error_message = 'Nursing ID is required';              $valid = false;
  }

  if ($valid) {
    $sql    = "SELECT u.UserID,u.Username,u.PasswordHash,u.Role,u.LinkedID FROM USER u WHERE u.Username='".
               $username."' AND u.Role='".$role."'";
    ...
    if ($result && $result->num_rows > 0) {
      ...
      if ($password === $user['PasswordHash']) {
          ...
          case 'doctor':
            $stmt = $conn->prepare(
              "SELECT d.DoctorID
               ...
               WHERE d.DoctorID=? AND u.Username=?"
            );
            $stmt->bind_param('is', $user['LinkedID'], $username);
            $stmt->execute();
            $dr = $stmt->get_result();
            $role_verified = ($dr && $dr->num_rows > 0);
            $stmt->close();
            break;
          case 'nurse':
            $n_sql = "SELECT NurseID FROM NURSE WHERE NurseID=".$user['LinkedID']." AND Username='".$username."'";
            $nr    = $conn->query($n_sql);
            $role_verified = ($nr && $nr->num_rows > 0);
            break;
          case 'patient':
            $role_verified = true;
            break;
        ...
        if ($role_verified) {
          // Cập nhật LastLogin
          $conn->query("UPDATE USER SET LastLogin=NOW() WHERE UserID=".$user['UserID']);
          // Ghi audit log
          $ip = $_SERVER['REMOTE_ADDR'];
          $conn->query(
            "INSERT INTO AUDIT_LOG(UserID,Timestamp,Action,TableAffected,IPAddress) VALUES(".
            $user['UserID'].",NOW(),'Login','USER','".$ip."')"
          );
          ...
          switch ($role) {
            ...
            case 'doctor': header('Location: dashboard.php');     exit;
            case 'nurse':  header('Location: nurse/index.php');    exit;
